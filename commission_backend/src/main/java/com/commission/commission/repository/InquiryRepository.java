...
public interface InquiryRepository extends JpaRepository<Inquiry, Long> {
	...
	List<Inquiry> findByCommission_Id(Long commissionId);

	// 특정 부모 댓글에 달린 대댓글 조회
	List<Inquiry> findByParentId(Long parentId);

	// 특정 커미션 + 특정 부모 댓글 기준으로 조회
    List<Inquiry> findByCommission_IdAndParentId(Long commissionId, Long parentId);
